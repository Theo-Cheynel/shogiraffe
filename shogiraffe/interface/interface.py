...
from ..strategy.agents import neural_agent, neural_network, simple_agent
...
class UI:
    ...
    def __init__(self, board, agent):
        ...
        self.canvas = Canvas(
            master,
            width=9 * TILE_SIZE + 2 * X_OFFSET,
            height=9 * TILE_SIZE + 2 * Y_OFFSET,
            background="#ffeab0",
        )
    ...
    def display_board(self):
        ...
        for i in range(0, 10):
            ...
            self.canvas.create_line(
                X_OFFSET,
                TILE_SIZE * i + Y_OFFSET,
                X_OFFSET + TILE_SIZE * 9,
                TILE_SIZE * i + Y_OFFSET,
            )
            # Vertical lines
            self.canvas.create_line(
                TILE_SIZE * i + X_OFFSET,
                Y_OFFSET,
                TILE_SIZE * i + X_OFFSET,
                Y_OFFSET + 9 * TILE_SIZE,
            )
            ...
            for colonne in range(9):
                ...
                if a != None:
                    ...
                    self.canvas.create_text(
                        x,
                        y,
                        font=("CODE2000", int(TILE_SIZE / 3)),
                        fill=color,
                        text=a.japanese_symbol(),
                    )
        ...
        for i in l:
            ...
            self.canvas.create_text(
                ...
                font=("CODE2000", int(TILE_SIZE / 3)),
                fill=color,
                text=JAPANESE_SYMBOLS[PIECE_SYMBOLS[i]],
            )
            self.canvas.create_text(
                x + TILE_SIZE / 4,
                y + TILE_SIZE / 3,
                font=("CODE2000", int(TILE_SIZE / 6)),
                fill=color,
                text=str(l[i]),
            )
            c += 1
        ...
        if self.state == WaitingForHumanResponseToDialog:
            ...
            self.canvas.create_rectangle(
                X_OFFSET + 2.5 * TILE_SIZE,
                Y_OFFSET + 3.5 * TILE_SIZE,
                X_OFFSET + 6.5 * TILE_SIZE,
                Y_OFFSET + 5.5 * TILE_SIZE,
                fill="white",
            )

            # "Promotion ?"
            self.canvas.create_text(
                X_OFFSET + 4.5 * TILE_SIZE,
                Y_OFFSET + 4.1 * TILE_SIZE,
                font=("CODE2000", int(TILE_SIZE / 4)),
                fill="black",
                text="PROMOTE ?",
            )
            ...
            self.canvas.create_rectangle(
                ...
                Y_OFFSET + 4.7 * TILE_SIZE,
                X_OFFSET + 4.42 * TILE_SIZE,
                Y_OFFSET + 5.2 * TILE_SIZE,
                fill="white",
            )
            self.canvas.create_text(
                X_OFFSET + 3.7 * TILE_SIZE,
                Y_OFFSET + 4.97 * TILE_SIZE,
                font=("CODE2000", int(TILE_SIZE / 4)),
                fill="black",
                text="YES",
            )
            ...
            # "NO" button
            self.canvas.create_rectangle(
                X_OFFSET + 4.58 * TILE_SIZE,
                Y_OFFSET + 4.7 * TILE_SIZE,
                X_OFFSET + 6 * TILE_SIZE,
                Y_OFFSET + 5.2 * TILE_SIZE,
                fill="white",
            )
            self.canvas.create_text(
                X_OFFSET + 5.3 * TILE_SIZE,
                Y_OFFSET + 4.97 * TILE_SIZE,
                font=("CODE2000", int(TILE_SIZE / 4)),
                fill="black",
                text="NO",
